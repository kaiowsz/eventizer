...
const EventDetails = async ({params: {id}, searchParams}: SearchParamProps) => {
  ...
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl">
        ...
        <div className="flex w-full flex-col gap-8 p-5 md:p-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Link href={`${event.url}`} target="_blank" className="p-medium-16 lg:p-regular-18 text-primary-500 underline truncate">{event.url}</Link>
          </div>
    ...
    <section className="wrapper my-8 flex flex-col gap-8 md:gap-12">
      ...
      <Collection data={relatedEvents?.data} emptyTitle="No events found." emptyStateSubtext="Come back later" collectionType="All_Events" limit={3} page={searchParams.page as string} totalPages={relatedEvents?.totalPages || 1} />
    </section>
    </>
  )
}
